...
async function main() {
  if (!process.env.MONGO_URI || !process.env.MONGO_URI_CG) {
    throw new Error('MONGO_URI and MONGO_URI_CG are required in backend/.env');
  }

  const sourceConn = await mongoose.createConnection(process.env.MONGO_URI).asPromise();
  const targetConn = await mongoose.createConnection(process.env.MONGO_URI_CG).asPromise();
