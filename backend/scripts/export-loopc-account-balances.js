...
async function main() {
  ...
  if (!uri) throw new Error('Missing MONGO_URI_LOOPC')
  ...
  try {
    ...
    for (const row of ledgers) {
      ...
      if (!totals.has(drId) || !totals.has(crId)) continue
    ...
    const jsonPath = path.join(outDir, `loopc-account-balances-${stamp}.json`)
    const csvPath = path.join(outDir, `loopc-account-balances-${stamp}.csv`)
