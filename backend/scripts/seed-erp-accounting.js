...
const ledgerSeed = [
  { description: 'Initial capital funding', debit: '1010', credit: '3000', amount: 250000, referenceType: 'journal', department: 'finance', daysAgo: 21 },
  { description: 'Production raw material purchase', debit: '1200', credit: '2000', amount: 42000, referenceType: 'purchase', department: 'production', daysAgo: 18 },
  { description: 'Sales invoice settlement - Gulf Retail', debit: '1010', credit: '4000', amount: 88500, referenceType: 'invoice', department: 'sales', daysAgo: 14 },
  { description: 'Recognize cost of goods sold - April lot', debit: '5000', credit: '1200', amount: 21500, referenceType: 'inventory', department: 'production', daysAgo: 13 },
  { description: 'Monthly payroll accrual - HQ', debit: '6200', credit: '2100', amount: 18000, referenceType: 'payroll', department: 'hr', daysAgo: 11 },
  { description: 'Operations fleet and dispatch expense', debit: '6100', credit: '1010', amount: 9600, referenceType: 'expense', department: 'operations', daysAgo: 9 },
  { description: 'Vendor payment for bullion supplier', debit: '2000', credit: '1010', amount: 25000, referenceType: 'vendor_payment', department: 'finance', daysAgo: 7 },
  { description: 'Sales invoice settlement - Boutique partner', debit: '1010', credit: '4000', amount: 126000, referenceType: 'invoice', department: 'sales', daysAgo: 5 },
  { description: 'Executive office operating expense', debit: '6100', credit: '1010', amount: 7200, referenceType: 'expense', department: 'management', daysAgo: 3 },
  { description: 'Payroll accrual - factory team', debit: '6200', credit: '2100', amount: 22500, referenceType: 'payroll', department: 'production', daysAgo: 2 },
]
...
async function seed() {
  ...
  if (!actor) {
    throw new Error('No super admin user found. Create a super admin before seeding ERP data.')
  }
  ...
  for (const account of accounts) {
    const saved = await ChartOfAccount.findOneAndUpdate(
  ...
  for (const entry of ledgerSeed) {
    await Ledger.deleteMany({ description: entry.description })

    const seededDate = new Date()
    seededDate.setDate(seededDate.getDate() - (entry.daysAgo || 0))

    await Ledger.create({
      date: seededDate,
      debitAccountId: accountMap.get(entry.debit)._id,
      creditAccountId: accountMap.get(entry.credit)._id,
      amount: entry.amount,
  ...
  console.log('ERP accounting seed completed.')
  console.log(`Currencies: ${await Currency.countDocuments({ isActive: true })}`)
  console.log(`Accounts: ${await ChartOfAccount.countDocuments({ isActive: true })}`)
  console.log(`Mappings: ${await AccountMapping.countDocuments({ isActive: true })}`)
  console.log(`Ledger entries: ${await Ledger.countDocuments()}`)
  await mongoose.disconnect()
}

seed().catch(async (error) => {
  console.error('ERP accounting seed failed:', error.message)
  await mongoose.disconnect()
  process.exit(1)
})