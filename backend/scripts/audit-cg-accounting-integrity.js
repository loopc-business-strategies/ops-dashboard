...
async function main() {
  ...
  try {
    ...
    const [accounts, transactions, ledgers] = await Promise.all([
      db.collection('chartofaccounts').find({ isActive: true }).project({ _id: 1, accountCode: 1, accountName: 1, accountType: 1, isActive: 1 }).toArray(),
    ...
    for (const tx of postedTx) {
      ...
      if (typeNeedsBaseLedger(tx) && baseRows.length === 0) {
        voucherPostingIssues.push({
          txId: String(tx._id),
          type: tx.type,
          vocNo: tx?.voucherMeta?.vocNo || '',
          issue: 'Missing base ledger entry for posted voucher',
        })
      }
      ...
      if (!tx.debitAccountId || !tx.creditAccountId) {
        voucherPostingIssues.push({
          ...
          type: tx.type,
          vocNo: tx?.voucherMeta?.vocNo || '',
          issue: 'Posted transaction has missing debitAccountId or creditAccountId',
        })
      }
      ...
      if (['receipt', 'payment'].includes(String(tx.type || '').toLowerCase())) {
        ...
        if (!acCode) {
          voucherPostingIssues.push({
            ...
            type: tx.type,
            vocNo: tx?.voucherMeta?.vocNo || '',
            issue: 'Receipt/Payment has no selected settlement A/C code on line item',
          })
        } else {
          ...
          if (!coa) {
            voucherPostingIssues.push({
              ...
              type: tx.type,
              vocNo: tx?.voucherMeta?.vocNo || '',
              issue: `Receipt/Payment A/C code ${acCode} not found in active chart of accounts`,
            })
          }
        }
      }
    ...
    for (const row of jvRows) {
      if (!row.debitAccountId || !row.creditAccountId || Number(row.amount || 0) <= 0) {
        journalIssues.push({
          ...
          referenceId: row.referenceId ? String(row.referenceId) : '',
          amount: Number(row.amount || 0),
          issue: 'Journal row has missing debit/credit account or non-positive amount',
        })
      }
    }
    ...
    for (const [accId, totals] of accountTotals.entries()) {
      ...
      if (isFx) {
        ...
        fxDetails.push({
          ...
          gain: money(gain),
          loss: money(loss),
          net: money(netCredit - netDebit),
        })
      }
    }

    const profitBeforeTax = money(incomeTotal - expenseTotal)
    ...
    const output = {
      ...
      totals: {
        ...
        journalLedgers: jvRows.length,
      },
      postedByType: postedTx.reduce((acc, tx) => {
        const t = String(tx.type || 'unknown').toLowerCase()
        acc[t] = (acc[t] || 0) + 1
        return acc
      }, {}),
      trialBalance: {
        ...
        isBalanced: Math.abs(trial.imbalance) < 0.01,
      },
      profitLoss: {
      ...
      fx: {
        exchangeGainTotal: money(fxGainTotal),
        exchangeLossTotal: money(fxLossTotal),
        netFxImpact: money(fxGainTotal - fxLossTotal),
        details: fxDetails,
      },
      issues: {
        voucherPostingIssuesCount: voucherPostingIssues.length,
        voucherPostingIssues,
        journalIssuesCount: journalIssues.length,
        journalIssues,
        duplicateVoucherNumbersCount: duplicateVouchers.length,
        duplicateVoucherNumbers: duplicateVouchers,
      },
    }
    ...
    if (
      voucherPostingIssues.length > 0
      || journalIssues.length > 0
      || duplicateVouchers.length > 0
      || Math.abs(trial.imbalance) >= 0.01
    ) {
      process.exitCode = 2
    }
  } finally {
