...
async function main() {
  ...
  const rx = /\b(test|ooo|override)\b/i
  ...
  for (const check of checks) {
    ...
    const sample = await db.collection(check.collection).find(check.query).project({ _id: 1, accountCode: 1, accountName: 1, name: 1, isActive: 1, status: 1, description: 1, notes: 1, 'voucherMeta.partyName': 1, 'voucherMeta.partyCode': 1 }).limit(12).toArray()
