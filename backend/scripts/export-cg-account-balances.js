  ...
  try {
    ...
    const [accounts, ledgers] = await Promise.all([
      db.collection('chartofaccounts')
        .find({ isActive: true })
        .project({ _id: 1, accountCode: 1, accountName: 1, accountType: 1, currency: 1 })
        .toArray(),
      db.collection('ledgers')
        .find({ isDeleted: { $ne: true } })
        .project({ _id: 1, debitAccountId: 1, creditAccountId: 1, amount: 1 })
        .toArray(),
    ])
    ...
    const rows = [...totals.values()]
      .map((row) => {
        ...
        const balanceSide = net > 0 ? 'DR' : (net < 0 ? 'CR' : 'ZERO')
        return {
          ...row,
          debit,
          credit,
          net,
          netAbs: money(Math.abs(net)),
          balanceSide,
        }
      })
    ...
    const jsonPath = path.join(outDir, `cg-account-balances-${stamp}.json`)
    const csvPath = path.join(outDir, `cg-account-balances-${stamp}.csv`)
    ...
    csvLines.push(`"TOTAL","","","","${totalDebit.toFixed(2)}","${totalCredit.toFixed(2)}","${trialImbalance.toFixed(2)}","${Math.abs(trialImbalance) < 0.01 ? 'BALANCED' : 'UNBALANCED'}",""`)
