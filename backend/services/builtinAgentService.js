...
function buildAnalyzeReply(ctx) {
  ...
  const feed = feedLabel(metals)

  return [
    `# ${tenantUp} company report`,
    `Hi **${user.name}** — live snapshot for your dashboard.`,
    '',
    '## Summary',
    `- **Role:** ${user.role}${user.department ? ` · ${user.department}` : ''}`,
    `- **Build:** \`${(build?.commit || 'unknown').slice(0, 7)}\``,
    `- **Page:** ${pageContext?.tab || 'dashboard'}`,
    `- **Metals feed:** ${feed}${metals?.gold ? ` · Gold **${fmtNum(metals.gold, 2)}**` : ''}`,
    '',
    '## Key numbers',
    '| Area | Metric | Value |',
    '|------|--------|------:|',
    `| Tasks | Open | ${fmtNum(tasks.open)} |`,
    `| Tasks | Overdue | ${fmtNum(tasks.overdue)} |`,
    `| HR | Employees | ${fmtNum(hr.employees)} |`,
    `| ERP | Inventory | ${fmtNum(erp.inventoryItems)} |`,
    `| ERP | Low stock | ${fmtNum(erp.lowStock)} |`,
    `| ERP | Draft vouchers | ${fmtNum(erp.vouchers?.draft)} |`,
    `| ERP | Posted vouchers | ${fmtNum(erp.vouchers?.posted)} |`,
    `| CRM | Pipeline | ${fmtMoney(crm.pipelineValue)} |`,
    `| CRM | Hot leads | ${fmtNum(crm.hotLeads)} |`,
    `| CRM | Overdue follow-ups | ${fmtNum(crm.overdueFollowups)} |`,
    '',
    '## Alerts',
    ...
    '',
    '_Need code details? Say **Map project code** or **Fix …** with any error._',
  ].join('\n')
}
...
function runBuiltinAgent({ message, context, history = [] }) {
  ...
  if (intent === 'project') {
    return { reply: buildProjectStructureReply(), intent, mode: 'builtin' }
  }
