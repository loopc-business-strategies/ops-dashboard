/**
 * Run Gradle from mg-factory/android (no EAS). Usage:
 *   node scripts/gradle-android.mjs bundleRelease
 *   node scripts/gradle-android.mjs assembleRelease
 *
 * Path resolution (first match wins):
 *   OPS_MG_FACTORY_JUNCTION_ROOT/android
 *   OPS_DASHBOARD_REPO_ROOT/mg-factory/android
...
const mgFactoryJunction = process.env.OPS_MG_FACTORY_JUNCTION_ROOT
const repoRoot = process.env.OPS_DASHBOARD_REPO_ROOT
const androidDir = mgFactoryJunction
  ? path.resolve(mgFactoryJunction, 'android')
  : repoRoot
    ? path.resolve(repoRoot, 'mg-factory', 'android')
...
if (!fs.existsSync(path.join(dirNorm, isWin ? 'gradlew.bat' : 'gradlew'))) {
  console.error(
    `Native Android project not found at ${dirNorm}\n` +
      'Run first: npm run prebuild:android  (from mg-factory/) or npm run mg-factory:prebuild:android',
  )
  process.exit(1)
}
...
if (isWin) {
  const batPath = path.join(
    os.tmpdir(),
    `mg-factory-gradle-${process.pid}-${Date.now()}.bat`,
  )
  const batBody = [
    '@echo off',
    `cd /d "${winQuoteBat(dirNorm)}"`,
    'if errorlevel 1 exit /b 1',
    // Quote -P args so commas in reactNativeArchitectures are not cmd delimiters
    `call gradlew.bat ${task}${
      archGradleArg.trim() ? ` "${archGradleArg.trim()}"` : ''
    }${signingGradleArg.trim() ? ` "${signingGradleArg.trim()}"` : ''}`,
    'exit /b %ERRORLEVEL%',
  ].join('\r\n')
