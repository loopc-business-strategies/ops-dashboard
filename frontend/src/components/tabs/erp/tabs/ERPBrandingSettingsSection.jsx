...
export default function ERPBrandingSettingsSection({
  C,
  selectedBrandingKey,
    ...
    <>
      <div style={{ marginBottom: '1.25rem', background: C.p1, padding: '1rem', borderRadius: '0.5rem', border: `1px solid ${C.p2}` }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 1.2fr) repeat(3, minmax(130px, 0.7fr)) auto', gap: '0.65rem', alignItems: 'center' }}>
          ...
          <input type="number" min="80" max="260" placeholder="Width" value={brandingForm.logoWidth} onChange={(e) => setBrandingForm((prev) => ({ ...prev, logoWidth: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
          <input type="number" min="32" max="120" placeholder="Height" value={brandingForm.logoHeight} onChange={(e) => setBrandingForm((prev) => ({ ...prev, logoHeight: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
          <select value={brandingForm.logoFit} onChange={(e) => setBrandingForm((prev) => ({ ...prev, logoFit: e.target.value }))} style={ERP_MODAL_INPUT_STYLE}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        <form onSubmit={handleSaveBranding} style={{ background: C.p1, padding: '1rem', borderRadius: '0.5rem', border: `1px solid ${C.p2}`, marginBottom: '1rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 1.2fr) repeat(2, minmax(180px, 1fr))', gap: '0.75rem', marginBottom: '0.75rem' }}>
            ...
            <input placeholder="Profile Key" value={brandingForm.key} onChange={(e) => { const nextKey = normalizeBrandingKey(e.target.value); setSelectedBrandingKey(nextKey); setBrandingForm((prev) => ({ ...prev, key: nextKey })) }} style={ERP_MODAL_INPUT_STYLE} />
            <label style={{ ...ERP_MODAL_INPUT_STYLE, display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: 0 }}>
              <input type="checkbox" checked={Boolean(brandingForm.isDefault)} onChange={(e) => setBrandingForm((prev) => ({ ...prev, isDefault: e.target.checked }))} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.75rem', marginBottom: '0.75rem' }}>
            <input placeholder="Entity Name" value={brandingForm.entityName} onChange={(e) => setBrandingForm((prev) => ({ ...prev, entityName: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Branch / Unit" value={brandingForm.branchName} onChange={(e) => setBrandingForm((prev) => ({ ...prev, branchName: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Company Name" value={brandingForm.companyName} onChange={(e) => setBrandingForm((prev) => ({ ...prev, companyName: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Legal Name" value={brandingForm.legalName} onChange={(e) => setBrandingForm((prev) => ({ ...prev, legalName: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Company Address" value={brandingForm.address} onChange={(e) => setBrandingForm((prev) => ({ ...prev, address: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Company Phone" value={brandingForm.phone} onChange={(e) => setBrandingForm((prev) => ({ ...prev, phone: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="TRN / Tax Registration" value={brandingForm.trn} onChange={(e) => setBrandingForm((prev) => ({ ...prev, trn: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Report Subtitle" value={brandingForm.reportSubtitle} onChange={(e) => setBrandingForm((prev) => ({ ...prev, reportSubtitle: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Footer Text" value={brandingForm.reportFooter} onChange={(e) => setBrandingForm((prev) => ({ ...prev, reportFooter: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '0.75rem', alignItems: 'start', marginBottom: '0.75rem' }}>
            <input placeholder="Logo URL or paste data URL" value={brandingForm.logoUrl} onChange={(e) => setBrandingForm((prev) => ({ ...prev, logoUrl: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <label style={{ ...ERP_MODAL_INPUT_STYLE, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', marginBottom: 0 }}>
              Upload Logo
              <input type="file" accept={LOGO_UPLOAD_ACCEPT} onChange={(e) => handleBrandingLogoFile(e.target.files?.[0])} style={{ display: 'none' }} />
            </label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.75rem', marginBottom: '1rem' }}>
            <input type="number" min="80" max="260" placeholder="Logo Width" value={brandingForm.logoWidth} onChange={(e) => setBrandingForm((prev) => ({ ...prev, logoWidth: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input type="number" min="32" max="120" placeholder="Logo Height" value={brandingForm.logoHeight} onChange={(e) => setBrandingForm((prev) => ({ ...prev, logoHeight: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <select value={brandingForm.logoFit} onChange={(e) => setBrandingForm((prev) => ({ ...prev, logoFit: e.target.value }))} style={ERP_MODAL_INPUT_STYLE}>
              <option value="contain">Contain</option>
          ...
          <div style={{ marginBottom: '1rem', padding: '1rem', borderRadius: '0.75rem', border: `1px solid ${C.p2}`, background: '#F8FAFC' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.75rem', marginBottom: '1rem' }}>
            <input placeholder="Prepared By Title" value={brandingForm.preparedByTitle} onChange={(e) => setBrandingForm((prev) => ({ ...prev, preparedByTitle: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Prepared By Name" value={brandingForm.preparedByName} onChange={(e) => setBrandingForm((prev) => ({ ...prev, preparedByName: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Reviewed By Title" value={brandingForm.reviewedByTitle} onChange={(e) => setBrandingForm((prev) => ({ ...prev, reviewedByTitle: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Reviewed By Name" value={brandingForm.reviewedByName} onChange={(e) => setBrandingForm((prev) => ({ ...prev, reviewedByName: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Approved By Title" value={brandingForm.approvedByTitle} onChange={(e) => setBrandingForm((prev) => ({ ...prev, approvedByTitle: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
            <input placeholder="Approved By Name" value={brandingForm.approvedByName} onChange={(e) => setBrandingForm((prev) => ({ ...prev, approvedByName: e.target.value }))} style={ERP_MODAL_INPUT_STYLE} />
          </div>
