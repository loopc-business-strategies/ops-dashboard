...
export default function ERPCustomersTab({
  ...
  const colCount = 12
  const { scrollRef, enabled: virtEnabled, virtualItems, paddingTop, paddingBottom } = useVirtualTableRows(
    customers.length,
    { estimateSize: 48 },
  )
  ...
  const renderCustomerRow = (customer) => {
    ...
    return (
      <tr key={customer._id} style={{ borderBottom: `1px solid ${C.p2}`, contentVisibility: 'auto', containIntrinsicSize: 'auto 48px' }}>
        ...
        <td style={{ padding: '0.75rem', color: C.t2 }}>
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            <button type="button" onClick={() => handleEditCustomer(customer)} style={{ padding: '0.35rem 0.7rem', background: '#0F766E', color: '#fff', border: 'none', borderRadius: '0.35rem', cursor: 'pointer' }}>Edit</button>
            <button type="button" onClick={() => handleDeleteCustomer(customer)} style={{ padding: '0.35rem 0.7rem', background: C.danger, color: '#fff', border: 'none', borderRadius: '0.35rem', cursor: 'pointer' }}>Delete</button>
          </div>
        </td>
      </tr>
    )
  ...
  const visibleCustomers = virtEnabled
    ? virtualItems.map((v) => customers[v.index]).filter(Boolean)
    : customers
  ...
  return (
    <div>
      ...
      <div ref={scrollRef} style={{ overflow: 'auto', maxHeight: virtEnabled ? 560 : undefined, background: C.p1, borderRadius: '0.5rem' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
          ...
          <tbody>
            ...
            {visibleCustomers.map((customer) => renderCustomerRow(customer))}
