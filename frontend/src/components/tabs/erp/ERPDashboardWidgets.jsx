...
function MarginsWidget({
  ...
  const mapRow = (row, nameKey, options = {}) => mapErpLiveMarginRow(row, nameKey, {
    ...options,
    goldPriceUSD,
    silverPriceUSD,
    marginLiveRecalc,
  ...
  return (
    <div>
        ...
        <div style={{ display: 'grid', gap: '0.42rem', minWidth: 0 }}>
          {segments.map((seg) => (
            <div key={seg.label} style={{ display: 'flex', alignItems: 'center', gap: '0.42rem', fontSize: '0.74rem', color: '#64748B', minWidth: 0 }}>
              ...
              <span style={{ marginLeft: 'auto', color: '#334155', fontWeight: '700' }}>{seg.pct.toFixed(0)}%</span>
            </div>
          ))}
        </div>
      </div>
      <div style={{ marginTop: '0.85rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.55rem' }}>
        <div style={{ border: '1px solid #E5E7EB', borderRadius: '0.5rem', padding: '0.55rem' }}>
          <p style={{ margin: 0, color: '#64748B', fontSize: '0.66rem', fontWeight: '700' }}>THIS MONTH</p>
          <p style={{ margin: '0.16rem 0 0', color: '#111827', fontSize: '0.9rem', fontWeight: '900' }}>{fmtDollar(currentTotal)}</p>
        </div>
        <div style={{ border: '1px solid #E5E7EB', borderRadius: '0.5rem', padding: '0.55rem' }}>
          <p style={{ margin: 0, color: '#64748B', fontSize: '0.66rem', fontWeight: '700' }}>THIS YEAR</p>
          <p style={{ margin: '0.16rem 0 0', color: '#059669', fontSize: '0.9rem', fontWeight: '900' }}>{fmtDollar(ytdTotal)}</p>
        </div>
      </div>
      ...
      <div style={{ marginTop: '0.75rem', textAlign: 'center' }}>
        <button type="button" onClick={() => setDetailsOpen(true)} style={{ border: '1px solid #E9D5FF', background: '#FFFFFF', color: '#5B21B6', borderRadius: '0.5rem', padding: '0.42rem 0.8rem', fontSize: '0.76rem', fontWeight: '800', cursor: 'pointer' }}>
          View More Details
        </button>
      </div>
          ...
          <div style={{ width: 'min(1120px, 96vw)', maxHeight: '88vh', overflow: 'hidden', background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '0.8rem', boxShadow: '0 24px 70px rgba(15,23,42,0.28)', display: 'flex', flexDirection: 'column' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: '1rem', overflowY: 'auto' }}>
              ...
              <ExpenseRegisterSection
                ...
                onAfterLedgerOpen={() => setDetailsOpen(false)}
