...
function KPIOverview() {
  ...
  return (
    <div className="space-y-6">
      <SectionHeader title="Production Overview" sub="Live production status — no demo metrics" />
      ...
      {linesForUi.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-gray-700 bg-gray-900/40 px-4 py-10 text-center">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Connect production feeds or use Production Control Center for live floor data.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          <StatCard icon="🏭" label="Lines Running" value={`${activeLines}/${linesForUi.length}`} color="#22c55e" />
          <StatCard icon="📦" label="Total Output" value={totalOutput.toLocaleString()} sub={`Target: ${totalTarget.toLocaleString()}`} color="var(--purple)" />
          <StatCard icon="⚡" label="Avg OEE" value={avgOEE == null ? '—' : `${avgOEE}%`} sub="Overall Equipment Effectiveness" color="#3b82f6" />
          <StatCard icon="🎯" label="Quality Rate" value={avgQuality == null ? '—' : `${avgQuality}%`} color="#22c55e" />
          <StatCard icon="🔧" label="Open Work Orders" value="—" color="#eab308" />
        </div>
      )}
