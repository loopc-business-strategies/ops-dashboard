...
} from '../../../utils/loopcPayrollDisplay'
    ...
    <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          {runDetail && (
            <Card title={`Run · ${runDetail.label} · ${runDetail.status}`}>
              ...
              <DataTable title="Lines" headers={['Employee','Position','Joining','Period','Days','Monthly','Calculated','Prev Arrears','Curr Adv Ded','Prev Mo Adv Ded','Other Ded','Total Ded','Earned','Net Paid']}>
                {(runDetail.lines || []).map((l, i) => (
                  <tr key={String(l.employeeId) + i}>
                    ...
                    <Td>{money(salaryCalculatedOf(l))}</Td>
                    <Td>{l.previousArrears == null ? '—' : money(l.previousArrears)}</Td>
                    <Td>{money(currentMonthAdvanceDeductionOf(l))}</Td>
                    <Td style={{ color: C.yellow || '#b45309' }}>{money(previousMonthAdvanceDeductionOf(l))}</Td>
                    <Td>{money(otherDeductionsOf(l))}</Td>
                    <Td>{money(displayTotalDeductionsOf(l))}</Td>
      ...
      {tab === 'payslips' && (
        <div className="space-y-3">
          ...
          <DataTable title="Payslips" headers={['Number','Employee','Position','Period','Days','Monthly','Calculated','Prev Arrears','Curr Adv Ded','Prev Mo Adv Ded','Other Ded','Total Ded','Net Paid','Status','Actions']}>
            {payslips.map((p) => (
              <tr key={p._id}>
                ...
                <Td>{money(salaryCalculatedOf(p))}</Td>
                <Td>{p.previousArrears == null ? '—' : money(p.previousArrears)}</Td>
                <Td>{money(currentMonthAdvanceDeductionOf(p))}</Td>
                <Td style={{ color: C.yellow || '#b45309' }}>{money(previousMonthAdvanceDeductionOf(p))}</Td>
                <Td>{money(otherDeductionsOf(p))}</Td>
                <Td>{money(displayTotalDeductionsOf(p))}</Td>
      ...
      {tab === 'reports' && (
        <div className="space-y-3">
          <Card title="Payroll report">
            <p style={{ fontSize: 12, color: C.t3, marginBottom: 10 }}>
              LoopC presentation view. Prev Mo Adv Ded is the unpaid residual (`salaryBalance`) already reflected in Net Paid — not a second deduction or EmployeeAdvance.
            </p>
          </Card>
          <DataTable
          ...
          >
            ...
            {!payslips.length && (
              <tr><Td colSpan={12} style={{ color: C.t3 }}>No payslips loaded. Open Payslips or wait for refresh.</Td></tr>
            )}
          </DataTable>
        </div>
      )}
