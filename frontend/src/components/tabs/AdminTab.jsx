...
function PermissionsTab({ users, token, initialUserId, onRefresh }) {
  ...
  const toggleModuleOn = (modId) => {
    setPerms((p) => {
      ...
      if (modId === 'erp') return { ...p, erp: { on: true, subs: {} } }
      return { ...p, [modId]: { on: true, view: true, edit: false } }
    })
  }

  const toggleErpSub = (subId, chip) => {
    setPerms((p) => {
      const erp = p.erp || { on: true, subs: {} }
      const subs = erp.subs || {}
      const sub = subs[subId] || { view: false, edit: false }
      return { ...p, erp: { ...erp, subs: { ...subs, [subId]: { ...sub, [chip]: !sub[chip] } } } }
    })
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          <h3 className="text-base font-semibold text-white">Permission Manager</h3>
          <p className="text-gray-300 text-sm mt-0.5">Control which modules and features each user can access.</p>
        </div>
      </div>

      <div className="flex gap-4" style={{ minHeight: '480px' }}>
        {/* Left: user list */}
        <div className="w-56 flex-shrink-0 border border-gray-800 rounded-2xl overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-gray-800 bg-gray-900/60 flex-shrink-0">
            <p className="text-xs font-semibold text-gray-300 uppercase tracking-wider">Users</p>
          </div>
          <div className="overflow-y-auto flex-1">
            {selectableUsers.map((u) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-all border-b border-gray-800/50 ${selectedUserId === u._id ? 'bg-emerald-700/10 border-l-2 border-l-emerald-700' : 'hover:bg-gray-800/40'}`}
              >
                <div className="w-8 h-8 bg-emerald-700/20 rounded-lg flex items-center justify-center text-emerald-400 font-bold text-xs flex-shrink-0">
                  {u.name?.[0]?.toUpperCase()}
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-medium text-white truncate">{u.fullName || u.name}</p>
                  <p className="text-xs text-gray-300 truncate capitalize">{u.role?.replace(/_/g, ' ')}</p>
                </div>
              </button>
            ))}
            {selectableUsers.length === 0 && <p className="text-center py-8 text-sm text-gray-300">No users found.</p>}
          </div>
        </div>

        {/* Right: permissions panel */}
        <div className="flex-1 border border-gray-800 rounded-2xl overflow-hidden flex flex-col">
          {!selectedUser ? (
            <div className="flex flex-col items-center justify-center h-full py-24 gap-3 text-gray-300">
            ...
            <>
              <div className="px-5 py-4 border-b border-gray-800 bg-gray-900/60 flex items-center justify-between gap-4 flex-shrink-0">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-emerald-700/20 rounded-lg flex items-center justify-center text-emerald-400 font-bold text-sm flex-shrink-0">
                    {selectedUser.name?.[0]?.toUpperCase()}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-white">{selectedUser.fullName || selectedUser.name}</p>
                    <p className="text-xs text-gray-300 capitalize">{selectedUser.role?.replace(/_/g, ' ')}{selectedUser.department ? ` · ${deptLabel(selectedUser.department)}` : ''}</p>
                  </div>
                </div>
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="px-4 py-2 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50 flex-shrink-0"
              ...
              <div className="overflow-y-auto flex-1 p-4 space-y-2">
                {PERM_MODULES.map((mod, i) => {
                  const isOn = !!perms[mod.id]?.on
                  const prevGroup = PERM_MODULES[i - 1]?.group
                  const showLabel = mod.group && mod.group !== prevGroup
                  return (
                    <div key={mod.id}>
                      {showLabel && <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest pt-3 pb-1">{mod.group}</p>}
                      <div className={`rounded-xl border px-4 py-3 flex items-center justify-between gap-3 transition-all ${isOn ? 'border-emerald-700/30 bg-emerald-700/5' : 'border-gray-800 bg-gray-900/40'}`}>
                        <span className="text-sm font-medium text-white">{mod.label}</span>
                        <button
                          onClick={() => toggleModuleOn(mod.id)}
                        ...
                        >
                          <span className={`inline-block h-4 w-4 rounded-full bg-white transition-transform ${isOn ? 'translate-x-4' : 'translate-x-0'}`} />
                        </button>
                      </div>
                    </div>
                  )
                ...
                {/* ERP with per-sub View/Edit chips */}
                <div>
                  <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest pt-3 pb-1">ERP</p>
                  <div className={`rounded-xl border transition-all ${isErpOn ? 'border-emerald-700/30 bg-emerald-700/5' : 'border-gray-800 bg-gray-900/40'}`}>
                    <div className="px-4 py-3 flex items-center justify-between gap-3">
                      <span className="text-sm font-semibold text-white">ERP</span>
                      <button
                        onClick={() => toggleModuleOn('erp')}
                        className={`relative inline-flex h-5 w-9 items-center rounded-full p-0.5 transition-all flex-shrink-0 ${isErpOn ? 'bg-emerald-700' : 'bg-gray-700'}`}
                      >
                        <span className={`inline-block h-4 w-4 rounded-full bg-white transition-transform ${isErpOn ? 'translate-x-4' : 'translate-x-0'}`} />
                      </button>
                    </div>
                    {isErpOn && (
                      <div className="border-t border-gray-800/60 divide-y divide-gray-800/40">
                        {ERP_PERM_SUBS.map((sub) => {
                          const sp = perms.erp?.subs?.[sub.id] || {}
                          return (
                            <div key={sub.id} className="flex items-center justify-between gap-3 px-4 py-2.5">
                              <span className="text-xs text-gray-300 pl-1">{sub.label}</span>
                              <div className="flex gap-2 flex-shrink-0">
                                <button
                                  onClick={() => toggleErpSub(sub.id, 'view')}
                                  className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all border ${sp.view ? 'bg-blue-500/20 border-blue-500/40 text-blue-300' : 'border-gray-700 text-gray-400 hover:border-gray-600'}`}
                                >
                                  View
                                </button>
                                <button
                                  onClick={() => toggleErpSub(sub.id, 'edit')}
                                  className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all border ${sp.edit ? 'bg-amber-500/20 border-amber-500/40 text-amber-300' : 'border-gray-700 text-gray-400 hover:border-gray-600'}`}
                                >
                                  Edit
                                </button>
                              </div>
