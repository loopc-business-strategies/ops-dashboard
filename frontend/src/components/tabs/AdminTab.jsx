...
const ROLE_COLOR = {
  super_admin: 'text-emerald-800 bg-emerald-50 border-emerald-200',
  management: 'text-blue-800 bg-blue-50 border-blue-200',
  department_head: 'text-amber-800 bg-amber-50 border-amber-200',
  department_user: 'text-gray-700 bg-gray-50 border-gray-200',
  external: 'text-green-800 bg-green-50 border-green-200',
}
...
function SettingTile({ title, desc, children }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm relative overflow-hidden">
      <div aria-hidden className="absolute left-0 top-0 right-0 h-0.5 bg-gradient-to-r from-violet-500 via-purple-500 to-emerald-500" />
...
function Toggle({ checked, onChange, label, desc }) {
  return (
    ...
    >
      ...
      <span className={`inline-flex h-6 w-11 items-center rounded-full p-1 transition-all ${checked ? 'bg-violet-600' : 'bg-gray-300'}`}>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Username <span className="text-emerald-700">*</span></label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">{isEdit ? 'Reset Password' : 'Password'} {!isEdit && <span className="text-emerald-700">*</span>}</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Job Title</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Phone</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Location</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Timezone</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Employee Code</label>
      ...
      <div>
        <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Role</label>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {ROLES.map((r) => (
            <label key={r.value} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${form.role === r.value ? 'border-violet-300 bg-violet-50' : 'border-gray-200 bg-white hover:bg-gray-50'}`}>
              <input type="radio" name={isEdit ? 'edit-role' : 'create-role'} value={r.value} checked={form.role === r.value} onChange={() => setForm((f) => ({ ...f, role: r.value, department: '', allowedModules: [] }))} className="accent-violet-600" />
              <div>
      ...
      {(form.role === 'department_head' || form.role === 'department_user') && (
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Department</label>
      ...
      {form.role === 'external' && (
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Allowed Modules</label>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {ALL_MODULES.map((mod) => (
              <label key={mod} className={`flex items-center gap-2 p-2.5 rounded-lg border cursor-pointer text-sm transition-all ${form.allowedModules.includes(mod) ? 'border-violet-300 bg-violet-50 text-violet-900' : 'border-gray-200 bg-white text-gray-700 hover:bg-gray-50'}`}>
                <input type="checkbox" checked={form.allowedModules.includes(mod)} onChange={() => toggleModule(mod)} className="accent-violet-600" />
      ...
      {form.role === 'department_user' && (
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Assigned Task IDs</label>
      ...
      <div>
        <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Access Notes</label>
...
function CreateUserForm({ token, onCreated, onCancel }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-6 mb-6 shadow-sm relative overflow-hidden">
      <div aria-hidden className="absolute left-0 top-0 right-0 h-0.5 bg-gradient-to-r from-violet-500 via-purple-500 to-emerald-500" />
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          <h3 className="text-base font-semibold text-gray-900 mb-1">Create New User</h3>
          <p className="text-gray-600 text-sm">Create user accounts with operational metadata, department ownership, and access notes.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button type="button" onClick={onCancel} className="px-5 py-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 text-sm font-medium rounded-lg transition-colors">Cancel</button>
        </div>
      </form>
    </div>
  )
...
function EditUserModal({ user: u, token, onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 overflow-y-auto">
      <div className="bg-white border border-gray-200 rounded-xl p-6 w-full max-w-4xl shadow-xl my-4">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="text-base font-bold text-gray-900">Edit User</h3>
            <p className="text-gray-600 text-xs mt-0.5">{u.name}</p>
          </div>
          <button type="button" onClick={onClose} className="text-gray-500 hover:text-gray-800 text-lg leading-none">×</button>
        </div>
        ...
        <div className="flex gap-3 mt-5">
          <button type="button" onClick={onClose} className="flex-1 py-2.5 text-sm font-medium text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-all">Cancel</button>
          <button type="button" onClick={handleSave} disabled={saving} className="flex-1 py-2.5 text-sm font-medium text-white rounded-lg transition-all disabled:opacity-50" style={{ background: 'var(--grad-brand)' }}>
...
function UsersTab({ users, token, onRefresh, onOpenPermissions }) {
  ...
  return (
    <div>
      {toast && <div className="fixed top-4 right-4 z-50 bg-white border border-gray-200 text-gray-800 px-4 py-3 rounded-xl text-sm shadow-lg">{toast}</div>}
      ...
      <div className="flex items-center justify-between gap-4 mb-5 flex-wrap">
        <div>
          <h3 className="text-base font-semibold text-gray-900 leading-tight">Users & Access</h3>
          <p className="text-gray-600 text-sm mt-1 leading-relaxed">Manage user accounts, departments, role profiles, and access metadata.</p>
        </div>
        {!showCreate && <button type="button" onClick={() => setShowCreate(true)} className="px-4 py-2 text-white text-sm font-medium rounded-lg transition-colors" style={{ background: 'var(--grad-brand)' }}>+ Add User</button>}
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 mb-5 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Search</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Role Filter</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Department Filter</label>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              {['User', 'Role', 'Department', 'Access Profile', 'Status', 'Actions'].map((h) => <th key={h} className="text-left px-4 py-3 text-xs font-medium text-gray-600 uppercase tracking-wider">{h}</th>)}
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {filteredUsers.map((u) => (
              <tr key={u._id} className="hover:bg-gray-50/80 transition-colors align-top">
                <td className="px-4 py-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900 leading-tight">{u.fullName || u.name}{u._id === me?.id && <span className="ml-2 text-xs" style={{ color: 'var(--purple)' }}>(you)</span>}</p>
                      <p className="text-xs text-gray-600">@{u.name}</p>
                      <p className="text-xs text-gray-600 mt-1 leading-relaxed">{u.title || 'No title'}{u.employeeCode ? ` | ${u.employeeCode}` : ''}</p>
                      <p className="text-xs text-gray-600 mt-1 leading-relaxed">{u.phone || 'No phone'}{u.location ? ` | ${u.location}` : ''}</p>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-4 py-3"><span className="text-xs text-gray-600">{deptLabel(u.department)}</span></td>
                <td className="px-4 py-3">
                  <div className="text-xs text-gray-600 space-y-1">
                    <p>Timezone: <span className="text-gray-800">{u.timezone || '—'}</span></p>
                    <p>Tasks: <span className="text-gray-800">{u.assignedTasks?.length || 0}</span></p>
                    <p>Modules: <span className="text-gray-800">{u.allowedModules?.length ? u.allowedModules.join(', ') : 'Default role access'}</span></p>
                    <p>Last Login: <span className="text-gray-800">{formatDate(u.lastLogin)}</span></p>
                  </div>
                </td>
                <td className="px-4 py-3">
                  <span className={`flex items-center gap-1.5 text-xs ${u.isActive ? 'text-emerald-700' : 'text-red-600'}`}>
                ...
                <td className="px-4 py-3">
                  {u._id !== me?.id ? (
                    <div className="flex gap-2 flex-wrap">
                      <button type="button" onClick={() => setEditUser(u)} className="px-3 py-1 text-xs font-medium rounded-lg transition-all border border-violet-200 text-violet-700 hover:bg-violet-50">Edit</button>
                      <button type="button" onClick={() => onOpenPermissions(u._id)} className="px-3 py-1 text-xs font-medium text-violet-700 border border-violet-200 rounded-lg hover:bg-violet-50 transition-all">Permissions</button>
                      <button type="button" onClick={() => handleToggle(u)} className={`px-3 py-1 text-xs font-medium rounded-lg transition-all border ${u.isActive ? 'text-amber-700 border-amber-200 hover:bg-amber-50' : 'text-emerald-700 border-emerald-200 hover:bg-emerald-50'}`}>{u.isActive ? 'Deactivate' : 'Activate'}</button>
                      <button type="button" onClick={() => handleDelete(u)} className="px-3 py-1 text-xs font-medium text-red-600 border border-red-200 rounded-lg hover:bg-red-50 transition-all">Delete</button>
                    </div>
                  ) : <span className="text-xs text-gray-500">Self account protected</span>}
                </td>
              </tr>
            ))}
          </tbody>
        </table>
        {filteredUsers.length === 0 && <div className="text-center py-10 text-gray-600 text-sm">No users match the current filters.</div>}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h3 className="text-base font-semibold text-gray-900 leading-tight">Admin Settings</h3>
          <p className="text-gray-600 text-sm mt-1 leading-relaxed">Configure the operating profile for user access, approvals, alerts, and onboarding defaults.</p>
        </div>
        <div className="flex gap-2">
          <button type="button" onClick={resetSettings} className="px-4 py-2 text-sm font-medium rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50 transition-all">Reset</button>
          <button type="button" onClick={saveSettings} className="px-4 py-2 text-sm font-medium rounded-lg text-white transition-all" style={{ background: 'var(--grad-brand)' }}>Save Settings</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        <SettingTile title="System Profile" desc="Base operational defaults used across admin-managed processes.">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Company Name</label>
              <input value={settings.companyName} onChange={(e) => setSettings((s) => ({ ...s, companyName: e.target.value }))} className="input-field" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Primary Region</label>
              <input value={settings.primaryRegion} onChange={(e) => setSettings((s) => ({ ...s, primaryRegion: e.target.value }))} className="input-field" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Default Timezone</label>
              <input value={settings.defaultTimezone} onChange={(e) => setSettings((s) => ({ ...s, defaultTimezone: e.target.value }))} className="input-field" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Document Warning Days</label>
        ...
        <SettingTile title="Security Defaults" desc="Define baseline controls for admin and team access.">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Session Timeout (minutes)</label>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-2 uppercase tracking-wider">Password Rotation (days)</label>
        ...
        <SettingTile title="Monitoring & Notifications" desc="Admin-facing awareness and reporting defaults.">
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
              <p className="text-sm font-medium text-gray-900">Role Coverage Snapshot</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-3">
                {ROLES.map((role) => (
                  <div key={role.value} className="rounded-lg border border-gray-200 px-3 py-2 bg-white">
...
function PermissionsTab({ users, token, initialUserId, onRefresh }) {
  ...
  return (
    <div>
      ...
      <div className="flex border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm" style={{ minHeight: '520px' }}>
        {/* Sidebar: user list */}
        <div className="w-52 border-r border-gray-200 flex-shrink-0 flex flex-col bg-gray-50/50">
          <div className="px-4 py-3 border-b border-gray-200 bg-white">
            <p className="text-[10px] font-medium tracking-widest text-gray-500 uppercase">Users</p>
          </div>
            ...
            {selectableUsers.map((u) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 text-left transition-all border-l-[3px] ${
                  selectedUserId === u._id ? 'border-l-violet-600 bg-violet-50' : 'border-l-transparent hover:bg-white'
                }`}
              >
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium flex-shrink-0 bg-violet-100 text-violet-800">
                  {u.name?.[0]?.toUpperCase()}
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">{u.fullName || u.name}</p>
                  <p className="text-xs text-gray-500 truncate capitalize mt-0.5">{u.role?.replace(/_/g, ' ')}</p>
                </div>
              </button>
            ))}
            {selectableUsers.length === 0 && <p className="text-center py-8 text-xs text-gray-500">No users.</p>}
          </div>
        </div>
          ...
          {!selectedUser ? (
            ...
            <>
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 flex-shrink-0 bg-white">
                <div>
                  <p className="text-sm font-medium text-gray-900">{selectedUser.fullName || selectedUser.name}</p>
                  <p className="text-xs text-gray-500 mt-0.5 capitalize">{selectedUser.role?.replace(/_/g, ' ')}{selectedUser.department ? ` · ${deptLabel(selectedUser.department)}` : ''}</p>
                </div>
                <button
                  type="button"
                  onClick={handleSave}
              ...
              <div className="overflow-y-auto flex-1 px-5 py-4 bg-gray-50/30">
                {['GENERAL', 'DEPARTMENTS', 'ERP'].map((group) => (
                  <div key={group}>
                    <p className="text-[11px] font-medium tracking-widest text-gray-500 uppercase mt-5 mb-2 first:mt-0">{group}</p>
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center justify-between px-4 py-3 mb-1.5 rounded-xl border transition-all ${
                            isOn ? 'border-violet-200 bg-violet-50/80' : 'border-gray-200 bg-white'
                          }`}
                        >
    ...
    <ModuleTabColumn>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20">
          <div className="w-6 h-6 rounded-full animate-spin border-4 border-gray-200 border-t-violet-600" />
        </div>
      ) : (
        <>
