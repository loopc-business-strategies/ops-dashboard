/**
 * Department card display: live model data preferred, section-29 demo fills gaps.
 * Does not change buildDashboardModel.
 */

import { matchDashboardDeptKey } from './departmentConfig'

const LOSS_PREVIEW = 3
...
export function resolveDeptCardDisplay(card = {}, batchMonitorRows = [], employeeRatings = []) {
  ...
  if (metalIn == null && batches.length) {
    ...
    if (batches.some((b) => hasNum(b.qtyIn))) metalIn = sum
  }
  if (metalOut == null && batches.length) {
    const sum = batches.reduce((s, b) => s + (Number(b.qtyOut) || 0), 0)
    if (batches.some((b) => hasNum(b.qtyOut))) metalOut = sum
  }
  ...
  if (!employees.length) {
    ...
    // Fill missing ratings from demo peers by index when live rating absent
    ...
    while (employees.length < 3 && demo.employees[employees.length]) {
      ...
      if (!nameSet.has(normName(d.name))) employees.push({ ...d })
      else break
    }
  }
