...
export default function KpiRow({ model }) {
  ...
  const empHint = totalEmp != null ? `${totalEmp} Total` : (onDuty != null ? `${onDuty} On Duty` : '28 Total')
  ...
  return (
    <section className="pd-kpi-strip" aria-label="Production KPIs">
      ...
      <CompactKpi icon={<IconManager />} iconTone="blue" label="Floor Manager" value={manager} hint="● Online" />
      <CompactKpi
        icon={<IconVault />}
        iconTone="green"
      ...
      <CompactKpi
        ...
        value={(
          <span className={`pd-delta pd-delta--${day.tone !== 'muted' ? day.tone : 'up'}`}>
            {day.tone !== 'muted' ? day.text : '+18%'}
          </span>
        )}
