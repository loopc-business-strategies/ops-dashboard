...
const hasProductionApiRewrite = rewrites.some((rewrite) => {
  ...
  return hasJson.includes('loopcstrategies\\\\.com') && hasJson.includes('mg|cg|loopc|venusbullions|app')
})
...
const failures = []
if (!hasProductionApiRewrite) failures.push('Missing production tenant API rewrite for mg/cg/loopc/venusbullions/app on *.loopcstrategies.com.')
