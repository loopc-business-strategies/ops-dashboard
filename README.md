...
**Linting:** `npm run lint` runs repository guardrails **and** strict ESLint on **all production `frontend/src/components/tabs/erp/**/*.{js,jsx}`** and **`frontend/src/api/**/*.js`** (tests excluded, `--max-warnings=0`), **a repo-wide `react-hooks/rules-of-hooks` check** under **`frontend/src`**, **zero-warnings ESLint** on **`FinanceTab.jsx`** / **`OperationsTab.jsx`**, an **extended-tabs** slice, and **`lint:eslint:erp-shell-tabs`** (**`ERPTab.jsx`** / **`VoucherTab.jsx`**; see **`package.json`**). **`npm run lint:eslint:repo`** still scans all of **`frontend/src`** for broader cleanup (legacy warnings allowed). For lint plus mobile typecheck plus backend fast tests locally, use **`npm run check:ci-parity`** (see **`docs/TESTING.md`**).
