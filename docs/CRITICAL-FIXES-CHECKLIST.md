...
| 3 | **Mongo backup verification** | **Interim path active (2026-09-16)** — `ATLAS_BACKUP_PHASE=deferred`; weekly mongodump **green** (GH artifacts). **Next for durable retention:** set Cloudflare R2 / `BACKUP_S3_*` then `npm run setup:mongo-backup-rollout -- --r2 --github` and `MONGO_BACKUP_S3_CONFIGURED=true`. **Strict Atlas later:** M10+ Cloud Backup + `ATLAS_*` secrets, then flip `ATLAS_BACKUP_PHASE=strict`. Connectivity + upload drills OK. |
